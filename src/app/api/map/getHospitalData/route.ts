...
export async function POST(request: Request, res: NextApiResponse) {
  ...
  const getNearbyZipCodes = await fetch(
    `http://api.geonames.org/findNearbyPostalCodesJSON?postalcode=${zipCode}&country=US&radius=24.2&maxRows=${geoNamesLimit}&username=${process.env.GEONAMES_USERNAME}`
  );
  try {
    if (getNearbyZipCodes.ok) {
      ...
      for (
        let i = 0;
        i < Math.min(geoNamesLimit, getNearbyZipCodesData.postalCodes.length);
        i++
      ) {
  ...
  const fetchCMSData = await fetch(
